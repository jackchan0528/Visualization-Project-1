...
var color = d3.scaleQuantize()
    .domain([0.00, 1.00])
    .range(d3.range(11).map(function(d) { var temp = "q" + d + "-11";
      // console.log(temp);
      return temp;
  }));
...
d3.csv("data/allresult.csv", function(error, csv) {
  ...
  var data = d3.nest()
    ...
    // .rollup(function(d) { return (d[0].Close - d[0].Open) / d[0].Open; }) // the mathematical logics
    .rollup(function(d) { return d[0].percentage; })
    .map(csv);
    // All the keys in data are put into array, for each key produce new key without dollar sign
    Object.keys(data).forEach(function(d) {
      var temp = d.substring(1);
      var temp2 = new Date(temp);
    //  console.log(temp2);
      data[formatdata(temp2)]=parseFloat(data[d]);
      delete data[d];
    })
    // console.log(data);
...
// console.log(data);
  rect.filter(function(d) { return d in data; })
      .attr("class", function(d) { var tp = "day " + color(data[d]);
        // console.log(tp);
        return tp;
     })
